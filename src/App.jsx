...
function Portal() {
  ...
  // Helper: convert Firestore collection snapshot to sorted array
  const snapToList = (snap, initList) => {
    if (snap.empty) return initList;
    return snap.docs.map(d => d.data()).sort((a, b) => (a.id > b.id ? 1 : -1));
  };

  useEffect(() => {
    let count = 0;
    const total = 8; // number of collections to load
    const markLoaded = () => { count++; if (count >= total) setLoaded(true); };

    const unsubs = [
      onSnapshot(collection(db, "sinistres"),       s => { setSinistres(snapToList(s, INIT_SINISTRES)); markLoaded(); }),
      onSnapshot(collection(db, "ag-voted"),        s => { setAgVoted(snapToList(s, INIT_AG_VOTED)); markLoaded(); }),
      onSnapshot(collection(db, "parties"),         s => { setPartiesCommunes(snapToList(s, INIT_PARTIES_COMMUNES)); markLoaded(); }),
      onSnapshot(collection(db, "preroga"),         s => { setCsPreroga(snapToList(s, INIT_CS_PREROGA)); markLoaded(); }),
      onSnapshot(collection(db, "travindiv"),       s => { setTravIndiv(snapToList(s, INIT_TRAV_INDIV)); markLoaded(); }),
      onSnapshot(collection(db, "urgent"),          s => { setUrgentActions(snapToList(s, INIT_URGENT_ACTIONS)); markLoaded(); }),
      onSnapshot(collection(db, "longterm"),        s => { setLongterm(snapToList(s, INIT_LONGTERM)); markLoaded(); }),

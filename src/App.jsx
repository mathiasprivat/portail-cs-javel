...
// ── Firebase helpers ───────────────────────────────────────────────────────────
// Save a full list to a Firestore collection (each item = one document)
const fbSetList = async (collName, list) => {
  for (const item of list) {
    await setDoc(doc(db, collName, String(item.id)), item);
  }
};

const fbSet = async (collName, id, data) => {
  await setDoc(doc(db, collName, String(id)), data);
};

const fbDelete = async (collName, id) => {
  await deleteDoc(doc(db, collName, String(id)));
};

// Local fallback helpers (used only for migration check)
const lsGetRaw = (key, fallback) => {
  try { const v = localStorage.getItem(key); return v ? JSON.parse(v) : fallback; } catch { return fallback; }
};
const lsSetRaw = () => {}; // no-op — Firebase is now the source of truth
...
function LinksManager() {
  ...
  const addLink = (vals) => {
    const item = { id: Date.now(), title: vals.title.trim(), url: vals.url.trim() };
    fbSet("links", item.id, item);
  };
  const removeLink = (id) => {
    fbDelete("links", id);
  };
...
function Portal() {
  ...
  // ── All shared state ────────────────────────────────────────────────────────
  const [sinistres,       setSinistres]       = useState(INIT_SINISTRES);
  const [agVoted,         setAgVoted]         = useState(INIT_AG_VOTED);
  const [partiesCommunes, setPartiesCommunes] = useState(INIT_PARTIES_COMMUNES);
  const [csPreroga,       setCsPreroga]       = useState(INIT_CS_PREROGA);
  const [travIndiv,       setTravIndiv]       = useState(INIT_TRAV_INDIV);
  const [urgentActions,   setUrgentActions]   = useState(INIT_URGENT_ACTIONS);
  ...
  // Track deliberately deleted ids per collection to prevent re-seeding
  const deletedIds = useRef({});

  const markDeleted = (collName, id) => {
    if (!deletedIds.current[collName]) deletedIds.current[collName] = new Set();
    deletedIds.current[collName].add(String(id));
  };

  const snapToList = (snap, initList, collName) => {
    const deleted = deletedIds.current[collName] || new Set();
    const fromFirestore = {};
    snap.docs.forEach(d => { fromFirestore[String(d.id)] = d.data(); });

    // Seed missing initial items — but skip deleted ones
    initList.forEach(item => {
      const sid = String(item.id);
      if (!fromFirestore[sid] && !deleted.has(sid)) {
        fbSet(collName, item.id, item).catch(() => {});
      }
    });

    // Merge: Firestore wins; omit deleted
    const allIds = new Set([
      ...initList.map(i => String(i.id)),
      ...Object.keys(fromFirestore),
    ]);
    return Array.from(allIds)
      .filter(id => !deleted.has(id))
      .map(id => fromFirestore[id] || initList.find(i => String(i.id) === id))
      .filter(Boolean)
  ...
  useEffect(() => {
    ...
    const unsubs = [
      onSnapshot(collection(db, "sinistres"), s => { setSinistres(snapToList(s, INIT_SINISTRES, "sinistres")); markLoaded(); }),
      onSnapshot(collection(db, "ag-voted"),  s => { setAgVoted(snapToList(s, INIT_AG_VOTED, "ag-voted")); markLoaded(); }),
      onSnapshot(collection(db, "parties"),   s => { setPartiesCommunes(snapToList(s, INIT_PARTIES_COMMUNES, "parties")); markLoaded(); }),
      onSnapshot(collection(db, "preroga"),   s => { setCsPreroga(snapToList(s, INIT_CS_PREROGA, "preroga")); markLoaded(); }),
      onSnapshot(collection(db, "travindiv"), s => { setTravIndiv(snapToList(s, INIT_TRAV_INDIV, "travindiv")); markLoaded(); }),
      onSnapshot(collection(db, "urgent"),    s => { setUrgentActions(snapToList(s, INIT_URGENT_ACTIONS, "urgent")); markLoaded(); }),
  ...
  // ── Write helpers ───────────────────────────────────────────────────────────
  const saveItem = (collName, item) => fbSet(collName, item.id, item);
  const removeItem = (collName, id) => { markDeleted(collName, id); return fbDelete(collName, id); };
  ...
  const setPptState = (next) => {
    setPptStateRaw(next);
    setDoc(doc(db, "shared", "ppt"), next).catch(() => {});
  };

  const saveAgenda = (next) => {
    setAgenda(next);
    setDoc(doc(db, "shared", "agenda"), next).catch(() => {});
  };

  // ── Loading screen ──────────────────────────────────────────────────────────
  if (!loaded) return (
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "linear-gradient(180deg,#1a2a3a 0%,#2C3E50 100%)" }}>
      <div style={{ textAlign: "center", color: "#F0EBE1", fontFamily: "Inter, system-ui, sans-serif" }}>
        <div style={{ fontSize: 40, marginBottom: 16 }}>🏛</div>
        <div style={{ fontSize: 16, fontWeight: 600, marginBottom: 8 }}>Chargement du portail…</div>
        <div style={{ fontSize: 13, color: "#7F8C8D" }}>Synchronisation en cours</div>
      </div>
    </div>
  );
            ...
            <div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 14 }}>
                <div>
                  ...
                  <input value={agenda.date} onChange={e => saveAgenda({ ...agenda, date: e.target.value })} style={inputStyle} />
                </div>
                <div>
                  <label style={labelStyle}>Lieu</label>
                  <input value={agenda.lieu} onChange={e => saveAgenda({ ...agenda, lieu: e.target.value })} style={inputStyle} />
                </div>
              </div>
              <div style={{ marginBottom: 32 }}>
                <label style={labelStyle}>Ordre du jour</label>
                <textarea value={agenda.points} onChange={e => saveAgenda({ ...agenda, points: e.target.value })}
          ...
          {tab === "actualites" && (
            <div>
              ...
              <SectionBlock title="Sinistres" icon="🚨" color="#c0392b">
                {sinistres.map(p => (
                  <ProjectCard key={p.id} project={p} onChange={mkUpdater("sinistres")}
                    onDelete={() => mkDeleter("sinistres")(p.id)}
                ...
                <InlineAdder color="#c0392b" buttonLabel="+ Déclarer un sinistre"
                  ...
                  onAdd={v => { const item = { id: Date.now(), title: v.title, owner: "", comment: "", files: [], steps: [] }; saveItem("sinistres", item); }}
                />
              </SectionBlock>
              ...
              <SectionBlock title="Travaux votés en AG — suivi chantier" icon="🗳" color="#5d8a3c">
                ...
                {agVoted.map(p => (
                  <ProjectCard key={p.id} project={p} onChange={mkUpdater("ag-voted")}
                    onDelete={() => mkDeleter("ag-voted")(p.id)}
                ...
                <InlineAdder color="#5d8a3c" buttonLabel="+ Ajouter un chantier voté"
                  ...
                  onAdd={v => { const item = { id: Date.now(), title: v.title, owner: "", budget: v.budget || "", resolution: v.resolution || "", comment: "", files: [], steps: [] }; saveItem("ag-voted", item); }}
                />
              </SectionBlock>
              ...
              <SectionBlock title="Problèmes dans les parties communes" icon="🏢" color="#7b5ea7">
                ...
                {partiesCommunes.map(p => (
                  <ProjectCard key={p.id} project={p} onChange={mkUpdater("parties")}
                    onDelete={() => mkDeleter("parties")(p.id)}
                ...
                <InlineAdder color="#7b5ea7" buttonLabel="+ Signaler un problème"
                  ...
                  onAdd={v => { const item = { id: Date.now(), title: v.title, owner: "", comment: "", files: [], steps: [] }; saveItem("parties", item); }}
                />
              </SectionBlock>
              ...
              <SectionBlock title="Prérogatives du CS — sans vote AG" icon="🔑" color={C.copper}>
                ...
                {csPreroga.map(p => (
                  <ProjectCard key={p.id} project={p} onChange={mkUpdater("preroga")}
                    onDelete={() => mkDeleter("preroga")(p.id)}
          ...
          {tab === "ag" && (
            <div>
              ...
              <SectionBlock title="Travaux individuels nécessitant un vote en AG" icon="🗺" color={C.steel}>
                ...
                {travIndiv.map(p => (
                  <TravIndivCard key={p.id} project={p} onChange={mkUpdater("travindiv")}
                    onDelete={() => mkDeleter("travindiv")(p.id)} />
                ))}
                <InlineAdder color={C.steel} buttonLabel="+ Ajouter une demande"
                  ...
                  onAdd={v => { const item = { id: Date.now(), title: v.title, demandeur: v.demandeur || "", comment: v.comment || "", files: [] }; saveItem("travindiv", item); }}
                />
              </SectionBlock>
            </div>
          )}
...
function Portal() {
  ...
  // Merge Firestore snapshot with initial data:
  // - Items in Firestore win (they have user edits)
  // - Items only in initList are seeded into Firestore so they appear for everyone
  const snapToList = (snap, initList, collName) => {
    const fromFirestore = {};
    snap.docs.forEach(d => { fromFirestore[String(d.id)] = d.data(); });

    // Seed any missing initial items into Firestore (fire-and-forget)
    initList.forEach(item => {
      if (!fromFirestore[String(item.id)]) {
        fbSet(collName, item.id, item).catch(() => {});
      }
    });

    // Merge: Firestore data wins; fall back to initList for unknown ids
    const allIds = new Set([
      ...initList.map(i => String(i.id)),
      ...Object.keys(fromFirestore),
    ]);
  ...
  // ── Write helpers ───────────────────────────────────────────────────────────
  const saveItem = (collName, item) => fbSet(collName, item.id, item);
  const removeItem = (collName, id) => fbDelete(collName, id);
